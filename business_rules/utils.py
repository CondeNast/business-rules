...
def export_rule_data(variables, actions):
    ...
    for variable_class in inspect.getmembers(operators,
                                             lambda x: getattr(x, 'export_in_rule_data', False)):
...
def float_to_decimal(float_type):
    """
    ...
    """
    _numerator, _denominator = float_type.as_integer_ratio()
    numerator, denominator = Decimal(_numerator), Decimal(_denominator)
    ctx = Context(prec=60)
